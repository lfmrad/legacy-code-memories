% ** Uso de la funcion:
% Introduce los cocientes de P(x) de mayor a menor grado entre ([ ... ]):
% Ejercicio propuesto en clase. 20121126 | Luis F. Martinez Andreu.
function result=raicesf(pol)
	...
	if(length(pol)==3)
		...
		% result(1,1)=(-b-sqrt(raiz))/(2*a);
		% result(2,1)=(-b+sqrt(raiz))/(2*a);
		
		result=[ ((-b-sqrt(raiz))/(2*a)) ; ((-b+sqrt(raiz))/(2*a)) ];
	else
